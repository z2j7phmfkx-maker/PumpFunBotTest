...
def format_trade_detail(token, calc, target_info):
    ...
    detail += f"<b>💰 Prix cible pour +15% net</b>\n"
...
def handle_command(command, args):
    ...
    if cmd == "/start":
        ...
        msg += "/set_amount [€] – Modifier montant\n"
        msg += "/set_target [%] – Modifier target profit\n\n"
        msg += "<b>Wallets:</b>\n"
        msg += "/add_wallet [addr] – Ajouter wallet\n"
        msg += "/remove_wallet [addr] – Retirer wallet\n"
        msg += "/list_wallets – Lister wallets\n"
        msg += "/clear_wallets – Tout supprimer\n\n"
    ...
    elif cmd == "/set_amount" and len(parts) > 1:
        try:
            config["trade_amount"] = float(parts[1])
            save_config()
            send_telegram(f"✅ Montant changé à {config['trade_amount']}€")
        except:
            send_telegram("❌ Erreur format")
    
    elif cmd == "/set_target" and len(parts) > 1:
        try:
            config["profit_target"] = float(parts[1]) / 100
            save_config()
            send_telegram(f"✅ Target changé à +{config['profit_target']*100:.0f}%")
        except:
            send_telegram("❌ Erreur format")
    
    elif cmd == "/add_wallet" and len(parts) > 1:
        addr = parts[1]
    ...
    elif cmd == "/remove_wallet" and len(parts) > 1:
        addr = parts[1]
