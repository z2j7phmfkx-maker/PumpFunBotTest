...
def get_pump_fun_tokens(user_id):
    """Récupère les VRAIS tokens du wallet via l'API Helius - SANS FALLBACK"""
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for i, tx in enumerate(txs[:5]):
                # Extrait les infos du token
                description = tx.get("description", f"Token {i+1}")
                token_mint = tx.get("mint", f"unknown_{i}")
                ...
                tokens.append({
                    ...
                    "ca": token_mint,
                    "entry": 0.00001 * (i+1),
                    "ath": 0.00001 * (i+1) * (1 + (0.1 + i*0.05)),
                    "url": f"https://pump.fun/coin/{token_mint}"
                })
...
def run_backtest(user_id):
    ...
    # Vérifie s'il y a des tokens réels
    if not tokens:
